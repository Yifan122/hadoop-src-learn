 ...
 *
 *
 * 是一个抽象类，为NameNode，DataNode提供抽象的存储服务。
 * 管理着当前节点所有存储的目录，每一个目录都是由StorageDirectory
 * 来管理，storageDirs代表着这个Storage所管理的所有的StorageDirectory
 */
...
public abstract class Storage extends StorageInfo {
   ...
   * 我们知道DataNode和 NameNode可以定义多个存储目录来存储数据，StorageDirectory定义了管理存储目录的通用方法，
   * 简而言之，就是StorageDirectory可以用于管理DataNode或者NameNode定义的存储目录。
   */
  @InterfaceAudience.Private
  public static class StorageDirectory implements FormatConfirmable {
    ...
    //独占锁，支持DataNode或者 NameNode独占某个存储目录
    ...
    public StorageState analyzeStorage(StartupOption startOpt, Storage storage)
      ...
      try { // 检查路径是否存在，如果不存在，并且当前的启动命令既不是格式化也不是HOTSWAP，则将状态切换成NON_EXISTENT
        if (!root.exists()) {
          // storage directory does not exist
          if (startOpt != StartupOption.FORMAT &&
              startOpt != StartupOption.HOTSWAP) {//HOTSWAP --》 hdfs支持热交换驱动，也就是说datanode可以在不关闭的情况下更换数据存储位置。
        ...
        // 如果这个路径存在，但是并不是目录，则切换状态NON_EXISTENT
        if (!root.isDirectory()) {
          LOG.warn(rootPath + "is not a directory");
          return StorageState.NON_EXISTENT;
        }
        if (!FileUtil.canWrite(root)) {//判断datanode对这个路径是否有写权限，否则NON_EXISTENT（注意，此处并不报错只是打印 警告信息）
      ...
      File versionFile = getVersionFile();//获取datanode存储数据路径下的VERSION
      boolean hasCurrent = versionFile.exists();//判断是否存在这个文件（VERSION）
      ...
      if (!(hasPreviousTmp || hasRemovedTmp
        ...
        // 判断是否存在VERSION这个文件
        if (hasCurrent)//如果存在，则说明当前的存储状态是正常的
