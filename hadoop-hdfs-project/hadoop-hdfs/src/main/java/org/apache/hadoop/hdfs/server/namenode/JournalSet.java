...
public class JournalSet implements JournalManager {
  ...
  public void selectInputStreams(Collection<EditLogInputStream> streams,
    ...
    // 遍历journals，得到每个JournalAndStream实例（多个journalNode，所以有多个流）
    for (JournalAndStream jas : journals) {
      ...
      try {
        // 利用jas流得到JournalManager实例，然后调用其selectInputStreams()方法，获得输入流，并放入allStreams输入流集合
  ...
  private void mapJournalsAndReportErrors(
    ...
    //TODO
    /**
     *1、FileJournalManager 把元数据写入namenode
     *2、QuorumJournalManager 把元数据写入journalNode
     * */
    for (JournalAndStream jas : journals) {
      try {
        //闭包的apply方法，就是调用外层的apply实现
        closure.apply(jas);
      } catch (Throwable t) {
        if (jas.isRequired()) {
