...
public class QuorumJournalManager implements JournalManager {
  ...
  public void finalizeLogSegment(long firstTxId, long lastTxId)
    ...
    /**
     * loggers.waitForWriteQuorum必须等待各个JournalNode的响应回复
  ...
  /**
   * @param fromTxnId lastTxnId + 1
   * */
  @Override
  public void selectInputStreams(Collection<EditLogInputStream> streams,
      long fromTxnId, boolean inProgressOk) throws IOException {
    //RemoteEditLogManifest: 从JournalNodeRpc处获取edit log的文件清单列表（此时只是通过RPC获取文件清单，还不是具体的内容）
    ...
    final PriorityQueue<EditLogInputStream> allStreams = 
        new PriorityQueue<EditLogInputStream>(64,
            JournalSet.EDIT_LOG_INPUT_STREAM_COMPARATOR);//EDIT_LOG_INPUT_STREAM_COMPARATOR（大文件优先比较器）

    //TODO
    for (Map.Entry<AsyncLogger, RemoteEditLogManifest> e : resps.entrySet()) {
      AsyncLogger logger = e.getKey();//每一个 AsyncLogger 对应于一个 JournalNode 节点
      RemoteEditLogManifest manifest = e.getValue();//每一个文件清单列表
      
      for (RemoteEditLog remoteLog : manifest.getLogs()) {
        //TODO 得到journalNode上edit log文件清单的url地址(通过journalNodeHttpServer获取到的editlog的地址)
        ...
        //将journalNode的输入流添加到PriorityQueue队列
        allStreams.add(elis);
      }
    }
