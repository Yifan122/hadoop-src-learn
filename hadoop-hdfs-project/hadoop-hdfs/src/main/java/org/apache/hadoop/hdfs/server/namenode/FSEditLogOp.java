...
public abstract class FSEditLogOp {
  ...
  public static class Reader {
    ...
    private FSEditLogOp decodeOp() throws IOException {
      limiter.setLimit(maxOpSize);//设置限流50M
      in.mark(maxOpSize);//数据输入流也要标记一下读取的限流值

      if (checksum != null) {//如果数据校验的类为空，则初始化校验（一般是CRC）
        checksum.reset();
      }

      byte opCodeByte;
      try {
        // 外层是一个while循环，所以不断的从输入流in中读取一个byte，即opCodeByte
      ...
      if (opCode == OP_INVALID) {//如果当前是无效状态，则做校验
        verifyTerminator();
        return null;
      }
      // 根据FSEditLogOpCodes对象opCode从cache中获取FSEditLogOp对象
      ...
      // 从输入流in中读入其他属性，并设置入FSEditLogOp实例op
