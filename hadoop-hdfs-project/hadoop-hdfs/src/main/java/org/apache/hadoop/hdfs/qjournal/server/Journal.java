...
public class Journal implements Closeable {
  ...
  public synchronized void finalizeLogSegment(RequestInfo reqInfo, long startTxId,
    ...
    checkFormatted();//是否格式化了journalNode
    checkRequest(reqInfo);//确保请求是正确的，并且是有序的

    boolean needsValidation = true;

    // finalizeLogSegment是将处于in-process状态的editlog改成完整edit文件，所以这里需要关闭正在写入的流
    ...
    if (elf.isInProgress()) {//如果EditLog处于inprogress
      if (needsValidation) {
      ...
      //然后把edits_inprogress_0000000000000435244 改成正常的edits文件
   ...
   */
  /**
   * @param sinceTxId lastTxnId + 1
   * */
  public RemoteEditLogManifest getEditLogManifest(long sinceTxId,
      boolean inProgressOk) throws IOException {
    // No need to checkRequest() here - anyone may ask for the list
    // of segments.
    //检查journalNode是否已经进行了格式化操作
    checkFormatted();
    //在journalNode上获取edit文件的列表清单（此时不包括in-process）
    List<RemoteEditLog> logs = fjm.getRemoteEditLogs(sinceTxId, inProgressOk);

    //把非in-process状态的edit文件加载到集合，最终生成文件清单列表
    if (inProgressOk) {
