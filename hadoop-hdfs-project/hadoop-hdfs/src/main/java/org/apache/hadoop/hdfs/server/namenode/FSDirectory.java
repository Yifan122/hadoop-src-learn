...
public class FSDirectory implements Closeable {
  ...
  //TODO
  INodeDirectory rootDir;//整个文件系统的根目录
  private final FSNamesystem namesystem;//Namenode的门面类，这个类主要支持对数据块进行的操作
  private volatile boolean skipQuotaCheck = false; //skip while consuming edits
  private final int maxComponentLength;
  private final int maxDirItems;
  private final int lsLimit;  // max list limit
  private final int contentCountLimit; // max content summary counts per run
  private final INodeMap inodeMap; // 记录根目录下所有的INode（INodeId ->  INode）
    ...
    try {
      ...
      BlockInfoContiguousUnderConstruction blockInfo =
        new BlockInfoContiguousUnderConstruction(
            ...
            targets);
      //TODO blockManager里面记录了这个block信息
      getBlockManager().addBlockCollection(blockInfo, fileINode);
      //TODO 将新产生的这个block，记录到这个文件节点下面
  ...
  String resolvePath(FSPermissionChecker pc, String path, byte[][] pathComponents)
    ...
    //校验：如果是/.reserved/raw为开头并且如果不是超级用户，那么抛异常
  ...
  /**
   * 如果路径不以/.reserved/为开头则返回null
   * 否则得到文件夹的二进制数组
   * eg.test_mkdir --->byte[t][e][s][t][_][m][k][d][i][r]
   */
