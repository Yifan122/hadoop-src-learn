...
public class DFSUtil {
  ...
  private static Map<String, InetSocketAddress> getAddressesForNameserviceId(
    ...
    //emptyAsSingletonNull(nnIds):如果dfs.ha.namenodes是空，则返回一个空集合
   ...
   *       1、首先，根据NameNode类的静态方法getAddress()从配置信息中获取默认地址defaultAddress；
   ...
                 2.2、如果配置了dfs.internal.nameservices，再获取dfs.nameservices，得到availableNameServices，
                      验证parentNameServices中的每个nsId在availableNameServices中是否都存在，
                      即参数dfs.internal.nameservices包含在参数dfs.nameservices范围内；
           3、调用getAddressesForNsIds()方法，利用conf、parentNameServices、defaultAddress等
              获取nameserviceId->{nameNodeId->InetSocketAddress}对应关系的集合addressList，并返回。
   */
  public static Map<String, Map<String, InetSocketAddress>>
    ...
    String defaultAddress;
    try {
      //1、获取namenode的地址（localhost:9000）
      defaultAddress = NetUtils.getHostPortString(NameNode.getAddress(conf));
    } catch (IllegalArgumentException e) {
      defaultAddress = null;
    }
    ...
    /**
     *  2、获取hdfs的内部命名服务：dfs.internal.nameservices，得到集合parentNameServices
     * 显式指定属于本地群集的名称服务
     * 例如：
     * 集群A：
    ...
    //3、获取nameserviceId->{nameNodeId->InetSocketAddress}对应关系的集合
    Map<String, Map<String, InetSocketAddress>> addressList =
  ...
  public static HttpServer2.Builder httpServerTemplateForNNAndJN(
    ...
    /**
     * httpServer2是hadoop自己封装的httpServer
     * 通过实例化HttpServer2后，然后向里面设置参数
     * */
