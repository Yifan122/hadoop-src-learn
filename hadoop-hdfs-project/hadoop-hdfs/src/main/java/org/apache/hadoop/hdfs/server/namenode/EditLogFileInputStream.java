...
public class EditLogFileInputStream extends EditLogInputStream {
  ...
  private void init(boolean verifyLayoutVersion)
    ...
    try {
      //log---》URLLog.getInputStream //TODO 装饰着模式
      fStream = log.getInputStream();//此时拿到的是大于fsimage文件事务ID的最新的edit文件流
      ...
      if (NameNodeLayoutVersion.supports(
        ...
        try {
          LayoutFlags.read(dataIn);//LayoutFlags代表FSImage和编辑日志可以支持或不支持的功能，与布局版本无关
        } catch (EOFException eofe) {
          throw new LogHeaderCorruptException("EOF while reading layout " +
              "flags from log");
        }
      }
      //TODO 创建FSEditLogOp.Reader对象，用于open状态下的：reader.readOp
  ...
  private FSEditLogOp nextOpImpl(boolean skipBrokenEdits) throws IOException {
    ...
    case UNINIT:// 如果为未初始化状态UNINIT
      try {
        //构造一个读取edit的文件流Reader，然后切换编辑日志文件输入流的状态判断state为OPEN
        init(true);
      } catch (Throwable e) {
      ...
      // 检测编辑日志文件输入流状态，此时不应为UNINI
      Preconditions.checkState(state != State.UNINIT);
      return nextOpImpl(skipBrokenEdits);
    case OPEN://当编辑日志文件输入流的状态被切换成OPEN后，紧接着就会走到这里
      // 调用FSEditLogOp.Reader的readOp()方法，读取操作符（reader是在UNINIT状态的时候给实例化的）
      //TODO
      op = reader.readOp(skipBrokenEdits);//（opCode状态、事务ID、rpcID）
      if ((op != null) && (op.hasTransactionId())) {
        long txId = op.getTransactionId();
        if ((txId >= lastTxId) &&
            (lastTxId != HdfsConstants.INVALID_TXID)) {//如果当前的事务ID大于等于上一次的事务ID，并且上一次的事务不是无效的
          //可能namenode会出现写入崩溃，那么会在文件末尾产生一些垃圾。所以要用总文件长度 - 垃圾内容的开始位置，剩下的就是垃圾内容，然后跳过
      ...
      return SecurityUtil.doAsCurrentUser(
          new PrivilegedExceptionAction<InputStream>() {
            ...
            public InputStream run() throws IOException {
              ...
              try {
                /**
                 * journalNode在启动的时候，会有一个JournalNodehttpServer
                 *
                 * namenode:NameNodeRpcServer  、 NameNodeHttpServer
                 * datanode：DataNodeRpcServer 、 DataNodeHttpServer
