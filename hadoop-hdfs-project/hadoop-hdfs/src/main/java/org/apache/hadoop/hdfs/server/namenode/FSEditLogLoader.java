...
public class FSEditLogLoader {
  ...
  long loadEditRecords(EditLogInputStream in, boolean closeOnExit,
    ...
    Arrays.fill(recentOpcodeOffsets, -1);//填充数组
    //加载image文件的最新事务ID
    long expectedTxId = expectedStartingTxId;
    long numEdits = 0;//记录读取的edit log文件数量
    long lastTxId = in.getLastTxId();//当前edit输入流的最新事务ID
    //记录edit事务ID和image事务ID的差值（记录还需要处理的事务数）
    long numTxns = (lastTxId - expectedStartingTxId) + 1;
    StartupProgress prog = NameNode.getStartupProgress();
    Step step = createStartupProgressStep(in);
    prog.setTotal(Phase.LOADING_EDITS, step, numTxns);//把还需要处理的事务数记录到统计进度类里面
    ...
    try {
      while (true) {
        try {
          ...
          try {
            //TODO
            // 从编辑日志输入流in中读取操作符op（这个op，就是namenode写editLog的时候void logEdit(final FSEditLogOp op)）
            op = in.readOp();
            // 如果操作符op为空（说明没有元数据），直接跳出循环，并返回
            if (op == null) {
              break;
            }
          } catch (Throwable e) {
          ...
          //从流里面读取到的edit log偏移量记录到 recentOpcodeOffsets数组
          ...
          if (op.hasTransactionId()) {
            ...
            if (op.getTransactionId() > expectedTxId) { //从流拿到的edit事务ID必须和image的事务ID一致
          ...
          if (op.hasTransactionId()) {//如果当前的op的事务id是有效的
            //更新lastAppliedTxId，代表当前已经同步过来的TxId
  ...
  /**
   * applyEditLogOp()方法的主题结构非常清晰，通过一个switch-case结构体判断op类型，然后对op执行对应的操作
   * */
  ...
  private long applyEditLogOp(FSEditLogOp op, FSDirectory fsDir,
    ...
    case OP_MKDIR: {
      ...
      inodeId = getAndUpdateLastInodeId(mkdirOp.inodeId, logVersion,
          lastInodeId);
      //然后在内存中维护自己的元数据（就是把日志作用到自己的元数据里面）
