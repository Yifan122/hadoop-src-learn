...
public class LeaseManager {
   ...
   * @return
   */
  /**该方法遍历所有租约并计算块数 , FSNamesystem读锁必须在调用此方法。*/
  synchronized long getNumUnderConstructionBlocks() {
    assert this.fsnamesystem.hasReadLock() : "The FSNamesystem read lock wasn't"
            + "acquired before counting under construction blocks";
    long numUCBlocks = 0;
    //Lease对应HDFS一个目录
    for (Lease lease : sortedLeases) {//遍历所有的目录
      for (String path : lease.getPaths()) {//获取目录
  ...
  synchronized void renewLease(Lease lease) {
    if (lease != null) {
      ...
      //2）：更新续约时间（不然会因为超时而被回收）
      lease.renew();
      //3）：在把续约加到数据结构里，代表新的续约
      sortedLeases.add(lease);
    }
  }
   ...
   ******************************************************/
  /**NameNode通过后台工作线程LeaseManager$Monitor来定期检查LeaseManager中的文件租约是否过期，
   * 如果过期就释放该文件租约*/
  ...
  synchronized boolean checkLeases() {
    ...
    try {
      //因为sortedLeases租约队列已经是按最近更新时间值排序好的，所以取出的Lease对象就是最旧的一个租约
    ...
    while(leaseToCheck != null) {
      ...
      if (!leaseToCheck.expiredHardLimit()) {
        /*
        * 如果连最老的契约都没有过期 ， 那么新的契约肯定不会过期 ， 直接break即可
        * */
        break;
      }
      ...
      for(String p : leasePaths) {
        ...
        } catch (IOException e) {//如果释放组员失败，则添加到移除列表
      ...
      //把当前租约路径转化成节点路径后 ， 进行移除租约记录的remove操作
      for(String p : removing) {
        removeLease(leaseToCheck, p);
      }
      //如果最老的契约过期，那么可以依次拿到第二、第三老的
