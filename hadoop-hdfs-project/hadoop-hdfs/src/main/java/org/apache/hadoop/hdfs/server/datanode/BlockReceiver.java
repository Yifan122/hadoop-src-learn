...
class BlockReceiver implements Closeable {
  ...
  private DataChecksum clientChecksum; //Block数据的校验器
  ...
  BlockReceiver(final ExtendedBlock block, final StorageType storageType,
    ...
    try{
      ...
      // 打开文件，准备接收数据块
      if (isDatanode) {//isDatanode = false（最开始的时候）
        // 数据块复制和数据块移动是由数据节点发起的，这是在tmp目录下创建block文件
        replicaHandler = datanode.data.createTemporary(storageType, block);
      } else {
        switch (stage) {
          // 对于客户端发起的写数据请求，最开始肯定是建立管道然后写数据
        case PIPELINE_SETUP_CREATE:
          //在rbw目录下创建数据块（/tmp/hadoop-sys/dfs/data/current）
  ...
  private int receivePacket() throws IOException {
    ...
    if (lastPacketInBlock || len == 0) {
      ...
      if (checksumReceivedLen > 0 && shouldVerifyChecksum()) {
        try {
          verifyChunks(dataBuf, checksumBuf);//CRC32对chunk进行校验
        } catch (IOException ioe) {
  ...
  void receiveBlock(
      DataOutputStream mirrOut, // 到下一个数据节点去的输出流（发送数据给下游节点）
      DataInputStream mirrIn,   // 从下一个数据节点来的输入流（下游返回的应答）
      DataOutputStream replyOut,  // 到前一个数据节点去的输出流（给上游节点的应答）
    ...
    try {
      //TODO 如果是客户端发起的写请求（此处即为数据块create），则启动PacketResponder发送ack
      if (isClient && !isTransfer) {
        responder = new Daemon(datanode.threadGroup, 
            new PacketResponder(replyOut, mirrIn, downstreams));
        responder.start(); // start thread to processes responses
      }
      // 然后通过一个while循环不断的接收数据包信息 , 同步接收packet，写block文件和meta文件
  ...
  class PacketResponder implements Runnable, Closeable {   
    ...
    public void run() {
      ...
      while (isRunning() && !lastPacketInBlock) {
        ...
        try {
          Packet pkt = null;
          long expected = -2;//记录packet的序列号
          PipelineAck ack = new PipelineAck();
          long seqno = PipelineAck.UNKOWN_SEQNO;
          long ackRecvNanoTime = 0;
          try {
            //不是管道中的最后一个数据块并且没错误（最开始的时候 ，type ==LAST_IN_PIPELINE  ）
            ...
            //如果是管道的最后一个数据块 || packet的序列号不是未知的
            if (seqno != PipelineAck.UNKOWN_SEQNO
                || type == PacketResponderType.LAST_IN_PIPELINE) {
              //TODO 从ackQueue里面读取数据包
              pkt = waitForAckHead(seqno);
              if (!isRunning()) {
                break;
              }
              expected = pkt.seqno;//从ackQueue里取出的数据包后得到seqno，赋值给expected
          ...
          //TODO 从ackQueue拿出这个packet后 ， 获取packet的状态
          Status myStatus = pkt != null ? pkt.ackStatus : Status.SUCCESS;
          //TODO 把当前的确认信息和下游的确认信息合并，然后发到上游节点
          sendAckUpstream(ack, expected, totalAckTimeNanos,
            (pkt != null ? pkt.offsetInBlock : 0),
            PipelineAck.combineHeader(datanode.getECN(), myStatus));
          //TODO 应答完备 ， 从ackQueue里移除
          if (pkt != null) {
