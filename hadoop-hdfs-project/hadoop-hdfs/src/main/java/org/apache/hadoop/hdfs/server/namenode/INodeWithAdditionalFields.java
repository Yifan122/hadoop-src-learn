 ...
 *
 * TODO INode里面存在一系列的抽象方法，主要是这个类来实现的：id、name、permission、modificationTime、accessTime等
 */
@InterfaceAudience.Private
public abstract class INodeWithAdditionalFields extends INode
    implements LinkedElement {
  /**
   * 对应着成员变量：private long permission = 0L;
   * 所以在获取用户 、用户组按位来取就可以
   * */
  static enum PermissionStatusFormat {
    MODE(null, 16),//前16位用来标识文件的模式信息，类似于linux中的777模式
    GROUP(MODE.BITS, 25),//中间25位代表用户组
    USER(GROUP.BITS, 23);//最后23位代表用户
    ...
    static String getUser(long permission) {
      //首先获取permission中最后的23位用来做用户标识
      final int n = (int)USER.BITS.retrieve(permission);
      //SerialNumberManager用来获取用户标识对应的用户名
  ...
  //TODO HDFS在2.6.0以后，将磁盘配额、正在构建和快照等功能抽象成INode等特性，可以将特性添加到INode上
