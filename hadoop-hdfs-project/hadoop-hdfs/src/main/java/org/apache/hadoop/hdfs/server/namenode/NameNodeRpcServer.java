...
class NameNodeRpcServer implements NamenodeProtocols {
   ...
   * 第一部分，实例化各种通信协议和服务对象，比如：负责创建目录、管理block、设置权限等操作的客户端同NameNode通信的
   　　　　　　协议服务——ClientNameNodeProtocolServerSideTranslatorPB、负责datanode的启动时向NameNode注册、
             发送心跳报告、block信息报告等操作的datanode同NameNode通信的协议服务——DatanodeProtocolServerSideTranslatorPB
     第二部分，实例化了一个监听datanode请求的rpc server，
             并且将第一部分实例化的各种ProtocolService同此rpc server进行绑定，
   　　　　　　用于处理rpc server监听到的来自datanode的各种rpc请求。
     第三部分，实例化了一个监听客户端请求的rpc server，
             并将第一部分实例化的各种ProtocolService同此rpc server进行绑定，
             用于处理监听到的来自客户端的rpc请求。
   * */
  public NameNodeRpcServer(Configuration conf, NameNode nn)
    ...
    //NameNode有一个工作线程池用来处理客户端的远程过程调用及集群守护进程的调用，默认线程数是10
    int handlerCount = 
 ...
 * */
    ...
    if (serviceRpcAddr != null) {
      ...
      int serviceHandlerCount =
        ...
        //TODO 第二部分 启动ServiceRpcServer
        //这个服务是用来namenode和datanode之间的调用
      this.serviceRpcServer = new RPC.Builder(conf)
          ...
          .build();
        //TODO 将前面实例化的各种协议service添加到这个监听客户端请求的RPC server
    ...
    //获取namenode的RPC地址，默认localhost:9000
    ...
    //TODO 第三部分 启动clientRpcServer
      // 这个服务主要是用户使用客户端与Namenode和datanode进行交互服务调用的
  ...
  public HdfsFileStatus create(String src, FsPermission masked,
    ...
    try {
      ...
      //TODO 在namespace中创建hdfs file并返回文件信息状态
  ...
  public LocatedBlock addBlock(String src, String clientName,
    ...
    //如果有要排除的datanode节点，那迭代，然后添加到excludedNodesSet里面
  ...
  public CheckpointSignature rollEditLog() throws IOException {
    checkNNStartup();//检查namenode是否已经启动
    namesystem.checkSuperuserPrivilege();//检查权限
